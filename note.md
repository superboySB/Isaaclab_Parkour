...
```sh
git clone -b uav https://github.com/superboySB/Isaaclab_Parkour
...
其中，play 就是可视化推理。要切换到自己训练得到的日志，只需要把 `--use_pretrained_checkpoint` 替换成
`--log_root /workspace/isaaclab/Isaaclab_Parkour/logs/rsl_rl/unitree_go2_parkour`（或其他日志路径），脚本会从该目录里自动挑最新 checkpoint。

**深度图调试**：学生任务在 PLAY 配置里自动开启深度调试窗口，方便本地预览；训练任务（`--task ...Student-Unitree-Go2-v0`）默认关闭该窗口以便在无显示的服务器上运行。如果需要放大调试视图，可设置 `DEPTH_DEBUG_VIEWER_SCALE=6 python scripts/rsl_rl/play.py ...`。
...
1. **Teacher：先训老师拿到参考权重**  
   ...
   - 训练过程中每 `save_interval(=100)` 轮会生成 `model_<iter>.pt`，结束时还会写入 `model_50000.pt`（最后一次迭代）。脚本不会自动挑 “最优” checkpoint，你可以用 `play.py --checkpoint /path/to/model_*.pt` 手动指定检查。
...
3. **评估或导出**  
   - 训完老师或学生后，使用 `play.py` 并通过 `--log_root` + `--checkpoint` 指向这些新日志，就能直接在本地查看表现；同时 `play.py` 启动时会自动导出 JIT/ONNX（见脚本内置导出逻辑）。
